...
public class DownloadAtual {
    ...
    public static void BaixarPortariasAtual() throws IOException {
        ArrayList<String> GetDado_1 = new ArrayList<>();
        ArrayList<String> GetDado_2 = new ArrayList<>();
        String LinkAtual = "https://ifrs.edu.br/ibiruba";
        Document doc = Jsoup.connect(LinkAtual).get();
...
//                NAO FEZ NADA AQUI  OUTRO HISTORIA
                ...
                for (Element link_segun : links_segundary) {
                    ...
                    if (link_segun.attr("href").contains(link.attr("href")) && (!GetDado_1.contains(link_segun.attr("href")))) {
                        ...
                        if (link_segun.text().equals("Boletim de Serviço") || link_segun.text().equals("Portaria")) {
                            ...
                            if (!link_segun.attr("href").contains("pdf")) {
                                ...
                                Elements subLink = Link_Tree.select("a[href]");
                                for (Element getlink : subLink) {
                                    System.out.println(" href : " + getlink);
                                    if (getlink.attr("href").contains(link_segun.attr("href")) && (!GetDado_1.contains(Link_Tree.attr("href")))) {
                                        GetDado_2.add(getlink.attr("href"));
                                    } else if (getlink.attr("href").contains(".pdf")) {
                                        //VAI BAIXAR ARQUIVOS
                                        String webFile = getlink.attr("href");
                                        DownloadAtual dow = new DownloadAtual();
                                        dow.PreparedDownload(webFile, getlink.text(), NumbeArchiv++);
                                    }
                                }
                            }
